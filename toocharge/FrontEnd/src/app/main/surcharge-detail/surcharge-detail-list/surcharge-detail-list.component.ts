  ...
  unsave(event){
    ...
    modalRef.result.then((result)=>{
      if(result == 'ok')
      {
        ...
        let delAndEdit = ` <i class="icon-trash-2 del"></i>
     ...
     if(sur.name!=""&&sur.inforSurcharge.ticketNumber!=""&&sur.inforSurcharge.taxCode!=""&&sur.inforSurcharge.price!=""&&sur.currency!=""&&sur.includeVAT!=""&&sur.VATrate!="")
      {
        ...
        $('table tbody #tr'+idTag+' .check').html(htmlCheckbox);
      }
      else
      {
        let htmlBan = `<input type="hidden" value = "ban"> <i class="fas fa-ban"></i>`;
        $('table tbody #tr'+idTag+' .check').html(htmlBan);
      }
